...
class Reports(commands.Cog):
    class Info:
        prefix = "?"
        embed_color = discord.Color.from_rgb(200, 70, 0)
        report_channel = 768685719363256330
    ...
    async def gen_case_embed(parsed_json, case_num) -> discord.Embed:
        ...
        return discord.Embed(color=discord.Color.from_rgb(200, 70, 0), description="```Case: {8}\nUser: {0}\nModerator: {1}\nSteamID64: {2}\nAll the offenses: {7}\n{3}Date: {4}\nReason: {5}\nNotes: {6}```".format(
                                 user_id,
                                 mod_id,
